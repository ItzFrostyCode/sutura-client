...
export default function LegacyPortfolioPhotoRedirect({ params }: Readonly<{ params: Promise<{ shop_id: string; item_id: string }> }>) {
  return (
...
function LegacyPortfolioPhotoContent({ params }: Readonly<{ params: Promise<{ shop_id: string; item_id: string }> }>) {
  ...
  useEffect(() => {
    ...
    router.replace(`/shop/${shopId}/catalog/${itemId}/photo${qs ? `?${qs}` : ''}`);
  }, [shopId, itemId, router, searchParams]);
