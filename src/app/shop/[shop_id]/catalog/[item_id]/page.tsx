...
import { Ruler, Info, ShieldCheck, Star, CheckCircle2, AlertCircle, ArrowLeft, Flag, Store, ChevronDown, ChevronUp, ChevronRight, Locate as LocateIcon, Loader2, Shirt, Sparkles } from 'lucide-react';
        ...
        {selectedImage ? (
          <>
            ...
            <button
              ...
              style={{ top: 60 }}
            />
            {/* Model to Fabric Toggle Switch Pill on the top photo */}
            {fabricImage && (
              <div className="absolute bottom-3.5 left-1/2 -translate-x-1/2 z-20 flex items-center p-1 bg-black/65 backdrop-blur-md rounded-full shadow-lg border border-white/20">
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
            ...
            <div className="mt-3 pt-3 border-t border-line space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-ink">
                  {isFabricActive ? 'Fabric Swatch' : 'Model View'}
                </span>
                <span className="text-[11px] font-semibold text-taupe capitalize">
                  {isFabricActive ? (item.material || 'Fabric Close-Up') : (displayColor ? `${displayColor} · Model` : 'Model View')}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2.5 overflow-x-auto pb-1 scrollbar-none">
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSelectedVariation(img.view_angle || 'Model View');
                      }}
                      className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition-all relative shrink-0 shadow-xs active:scale-95 cursor-pointer ${
                        isSelected
                          ? 'border-ink ring-2 ring-taupe/50'
                          : 'border-line hover:border-taupe opacity-80 hover:opacity-100'
                      }`}
                      title={`${item.name} Model View`}
                    >
                      <Image
                        src={getMediaUrl(img.image_url)}
                        alt={`${item.name} Model View`}
                        className="w-full h-full object-cover object-top"
                        fill
                        unoptimized
                      />
                      <span className="absolute bottom-0 inset-x-0 bg-ink/80 backdrop-blur-xs text-[8px] text-white text-center py-0.5 font-bold uppercase tracking-wider">
                        Model
                      </span>
                    </button>
                  );
                ...
                {fabricImage && (
                  <button
                    ...
                    title={`${item.material || 'Fabric'} Swatch`}
                  >
                    <Image
                      src={getMediaUrl(fabricImage)}
                      alt={`${item.material || 'Fabric'} Swatch`}
