...
function NewTicketUploadItem({ upload, onRemove }: NewTicketUploadItemProps) {
  return (
    <div className="flex items-center justify-between p-3 border border-line rounded-xl bg-white text-sm">
      <div className="flex items-center gap-2 min-w-0 flex-1">
        ...
        <div className="min-w-0 flex-1">
          <p className="font-medium text-ink truncate">{upload.name}</p>
          {upload.status === 'uploading' && (
            <div className="w-full bg-line h-1.5 rounded-full mt-1 overflow-hidden">
              <div className="bg-taupe h-full transition-all duration-300" style={{ width: `${upload.progress}%` }} />
            </div>
          )}
          {upload.status === 'success' && <p className="text-xs text-sage mt-0.5 font-medium">Uploaded successfully</p>}
      ...
      <button
        ...
        className="p-1.5 rounded-lg text-ink-muted hover:text-red-500 hover:bg-red-50 transition-colors ml-2 shrink-0"
      >
        <Trash2 size={16} />
      </button>
    </div>
...
export default function SupportNewTicket({
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto text-ink">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="p-2 rounded-lg hover:bg-sunken text-ink-muted transition-colors">
          <ArrowLeft size={18} />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-ink tracking-tight">New Support Ticket</h1>
          <p className="text-ink-muted text-sm mt-0.5">Describe your issue or request and our team will respond shortly.</p>
        </div>
      </div>

      <form onSubmit={onSubmit} className="bg-surface border border-line rounded-2xl p-6 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="ticket-type" className="block text-sm font-medium text-ink mb-1.5">Ticket Type <span className="text-red-500">*</span></label>
            <select
              id="ticket-type"
              value={form.type}
              onChange={e => setForm(p => ({ ...p, type: e.target.value }))}
              className="w-full border border-line rounded-lg px-3 py-2.5 text-sm text-ink bg-white focus:outline-none focus:ring-2 focus:ring-taupe/40"
            >
          ...
          <div>
            <label htmlFor="priority" className="block text-sm font-medium text-ink mb-1.5">Priority <span className="text-red-500">*</span></label>
            <select
              id="priority"
              value={form.priority}
        ...
        <div>
          <label htmlFor="subject" className="block text-sm font-medium text-ink mb-1.5">Subject <span className="text-red-500">*</span></label>
          {isCustomSubject ? (
            <div className="space-y-1.5">
              <input
                ...
                className="w-full border border-line rounded-lg px-3 py-2.5 text-sm text-ink placeholder:text-ink-faint bg-white focus:outline-none focus:ring-2 focus:ring-taupe/40"
              />
              <button
                type="button"
        ...
        <div>
          <label htmlFor="message" className="block text-sm font-medium text-ink mb-1.5">Details <span className="text-red-500">*</span></label>
          <textarea
            rows={6}
            id="message"
            ...
            className="w-full border border-line rounded-lg px-3 py-2.5 text-sm text-ink placeholder:text-ink-faint bg-white focus:outline-none focus:ring-2 focus:ring-taupe/40 resize-none"
          />
        </div>

        <div className="space-y-2">
          <label htmlFor="attachments-upload" className="block text-sm font-medium text-ink mb-1">Attachments (Images/Videos up to 50MB)</label>
          <input
            type="file"
          ...
          <button
            ...
            className="w-full border border-dashed border-line hover:border-taupe/40 bg-canvas rounded-xl p-4 text-center cursor-pointer hover:bg-canvas/50 transition-all focus:outline-none focus:ring-2 focus:ring-taupe/40"
          >
            <div className="flex flex-col items-center gap-1.5">
              <Paperclip size={20} className="text-ink-muted" />
              <span className="text-sm font-medium text-ink-body">Click to select images or video</span>
              <span className="text-xs text-ink-faint">Maximum file size: 50MB</span>
            </div>
          </button>
        ...
        <div className="flex items-center justify-end gap-3 pt-2 border-t border-line">
          <button type="button" onClick={onBack} className="px-4 py-2 text-sm text-ink-muted hover:text-ink transition-colors">
            Cancel
          </button>
          <button
            type="submit"
