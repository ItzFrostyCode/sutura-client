  ...
  const renderReviewsContent = () => {
    ...
    return (
      <div className="bg-surface rounded-2xl border border-line divide-y divide-line overflow-hidden shadow-2xs">
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
              ...
              <div className="flex flex-row md:flex-col items-start md:items-end gap-3 shrink-0">
                ...
                <button
                  ...
                  className="p-1.5 text-ink-muted hover:text-red-500 rounded-lg cursor-pointer"
                  title="Delete Review"
                >
                  <Trash2 size={16} />
                </button>
        ...
        {lastPage > 1 && (
          <div className="p-4 bg-canvas/30 flex items-center justify-between">
            <button
              type="button"
              disabled={page === 1}
              onClick={() => setPage(p => p - 1)}
              className="px-4 py-1.5 rounded-lg border border-line bg-surface text-sm disabled:opacity-50 cursor-pointer shadow-2xs"
            >
              Previous
            </button>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="rating-filter" className="text-xs text-ink-muted font-medium">Filter:</label>
          <select
            id="rating-filter"
            value={filterRating}
            onChange={e => {
            ...
            className="bg-surface border border-line rounded-xl px-3 py-1.5 text-xs text-ink outline-hidden cursor-pointer shadow-2xs"
          >
