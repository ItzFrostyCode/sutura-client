...
interface StoreVisibilityPillProps {
...
export default function StoreVisibilityPill({ storeVisible, toggleVisibility, visibilityLoading }: StoreVisibilityPillProps) {
  if (storeVisible === null) return null;

  return (
    <div
      title={storeVisible ? 'Customers can find and book your store.' : 'Your store is hidden from the public catalog.'}
      ...
      >
        {visibilityLoading
          ...
          : <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${
              storeVisible ? 'translate-x-[18px]' : 'translate-x-1'
            }`} />
        }
      </button>
    </div>
