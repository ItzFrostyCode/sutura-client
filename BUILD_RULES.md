...
Build SUTURA exactly as specified in [REQUIREMENTS.md](./REQUIREMENTS.md). That document is the
...
- **frontend-dev** — all frontend code (Next.js, React, TypeScript, Tailwind CSS, Zustand) and
  frontend unit tests.
- **backend-dev** — all backend code (Laravel PHP, RESTful APIs), PostgreSQL (Supabase) schema,
  Cloudflare R2 integration, seed data, and backend unit tests.
...
- All image assets (shop logos, banners, gallery photos, portfolio images) are uploaded to and
  served from Cloudflare R2. No local image storage in the repository.
- API endpoints follow RESTful conventions: `GET /api/shops`, `POST /api/orders`,
  `PATCH /api/orders/{id}/status`, etc.
- Database migrations are versioned and reversible. Seed scripts are idempotent.
